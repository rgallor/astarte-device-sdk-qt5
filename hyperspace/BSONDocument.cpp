...
namespace Util
{
    ...
    static unsigned int bson_next_item_offset(
    ...
    static int bson_check_validity(const void *document, unsigned int fileSize)
    {
        ...
        if (docLen > fileSize) {
            qCWarning(bsonDocumentDC) << "BSON document is bigger than data: data: " << fileSize
                                      << " document: " << docLen;
            return 0;
        }
    ...
    QVariant BSONDocument::value(const char *name, QVariant defaultValue) const
    {
        ...
        switch (type) {
            ...
            case TYPE_DATETIME:
                return QVariant(
                    QDateTime::fromMSecsSinceEpoch(bson_value_to_int64(value)).toLocalTime());
    ...
    QHash<QByteArray, QByteArray> BSONDocument::byteArrayValuesHash() const
    {
        ...
        for (const void *item = bson_first_item(m_doc.constData()); item != nullptr;
            item = bson_next_item(m_doc.constData(), item)) {
    ...
    QList<QVariant> BSONDocument::listVariantValue(
        const char *name, const QList<QVariant> &defaultValue) const
    {
        QList<QVariant> tmp;
        uint8_t type;
        const void *value = bson_key_lookup(name, m_doc.constData(), &type);
        ...
        for (const void *item = bson_first_item(document.m_doc.constData()); item != nullptr;
            item = bson_next_item(document.m_doc.constData(), item)) {
    ...
    QHash<QByteArray, QVariant> BSONDocument::mapVariantValue(
        const char *name, const QHash<QByteArray, QVariant> &defaultValue) const
    {
